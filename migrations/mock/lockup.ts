...
export async function lockup(
	...
	await lockup.lockup(addressInfo[0].property!, new BigNumber(20000 * decimals), {from:addressInfo[0].account})
	const value = await lockup.getValue(addressInfo[0].property!, addressInfo[0].account)
	console.log(value)
}
