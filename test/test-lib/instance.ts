...
export class DevProtocolObject {
	private readonly _deployer: string

	private _registry!: RegistryInstance
	private _allocator!: AllocatorInstance
	private _dev!: DevInstance
	private _lockup!: LockupInstance
	private _propertyFactory!: PropertyFactoryInstance
	...
	public get registry(): RegistryInstance {
	...
	public async generateRegistry(): Promise<void> {
		const instance = contract('Registry')
		this._registry = await instance.new(this.fromDeployer)
	}
	...
	public async generateDev(): Promise<void> {
		...
		await this._registry.set('Token', this._dev.address, this.fromDeployer)
	}
	...
	public async generateLockup(): Promise<void> {
		...
		const block = await getBlock()
		await this._registry.set('Lockup', this._lockup.address, this.fromDeployer)
		await this._lockup.createStorage()
		await this._lockup.setDIP4GenesisBlock(block)
	}
	...
	public async generatePolicy(
		...
		const policy = await contract(policyContractName).new()
		this._treasury = await contract('TreasuryTest').new(this._registry.address)
		await policy.setTreasury(this._treasury.address)
		await this._policyFactory.create(policy.address)
		return policy.address
	}
	...
	public async getMarket(
		...
		const tmp = await contract(contractName).new(this._registry.address, {
			from: user,
		})
		return tmp
	}
