...
contract(
	'PolicyFactory',
	([deployer, dummyPolicy, user1, ...accounts]) => {
		before(async () => {
			await collectsEth(deployer)(accounts)
		})
		const init = async (): Promise<
			[DevProtocolInstance, IPolicyInstance]
		> => {
		...
		describe('PolicyFactory; create', () => {
			...
			it('If the policy already exists, it will not be applied.', async () => {
				...
				const secoundPolicy = await dev.getPolicy('PolicyTestForPolicyFactory', user1)
		...
		describe('PolicyFactory; forceAttach', () => {
			describe('failed', () => {
				...
				it('deadline is over.', async () => {
					...
					expect(curentPolicyAddress).to.be.equal(policy.address)
					await mine(10)
					const result = await dev.policyFactory.forceAttach(secoundPolicy.address).catch((err: Error) => err)
