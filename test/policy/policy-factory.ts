...
contract('PolicyFactory', ([deployer, user1, propertyAuther, ...accounts]) => {
	...
	describe('PolicyFactory; getVotingGroupIndex', () => {
		const policyContract = artifacts.require('Policy')
		let firstPolicyInstance: PolicyInstance
		let createdPropertyAddress: string
		beforeEach(async () => {
		...
		it('You can get an election group index.', async () => {
			const idx = await dev.policyFactory.getVotingGroupIndex()
			expect(idx.toNumber()).to.be.equal(0)
		})
		it('When convergePolicy is executed, the index is raised.', async () => {
			await dev.dev.deposit(createdPropertyAddress, 10000, {from: user1})
			policy = await dev.getPolicy('PolicyTestForPolicyFactory', user1)
			const policyCreateResult = await dev.policyFactory.create(
				policy.address,
			...
			const secondPolicyAddress = getPolicyAddress(policyCreateResult)
			await dev.voteCounter.votePolicy(
				secondPolicyAddress,
				createdPropertyAddress,
				true,
			...
			expect(await dev.addressConfig.policy()).to.be.equal(secondPolicyAddress)
		})
	})
})
