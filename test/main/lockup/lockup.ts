...
describe('Lockup', () => {
	...
	const init = async (initialUpdate = true): Promise<DevProtocolInstance> => {
		...
		await dev.policy.create(PolicyTestForLockup)
		await dev.property.create('test', 'TEST')

		await dev.addressConfig.setMetricsFactory(dev.deployer.address)
		await dev.metricsGroup.addGroup(dev.deployer.address)
	...
	describe('Lockup; cancel', () => {
		it('An error occurs if you specify something other than a property address.', async () => {
			...
			const res = await dev.lockup.cancel(wallet2.address).catch(err)
			validateAddressErrorMessage(res)
		})
		it('An error will occur if not locked up.', async () => {
			const dev = await init()
			const property = dev.property.getOne()!
