...
const handler = async (
	...
	await nextWithdrawStorage.setStorage(withdrawStorageAddress, {
	...
	// Activation
	await withdrawStorage.changeOwner(nextWithdrawStorage.address, {
		gasPrice: await fastest(),
		gas,
	})
	____log('Delegated EternalStorage owner to the new WithdrawStorage')

	await config.setWithdrawStorage(nextWithdrawStorage.address, {
		gasPrice: await fastest(),
		gas,
	})
