...
library BokkyPooBahsDateTimeLibrary {
	...
	function timestampFromDateTime(
		...
		timestamp = _daysFromDate(
			year,
			month,
			day
		) * SECONDS_PER_DAY + hour * SECONDS_PER_HOUR + minute * SECONDS_PER_MINUTE + second;
	}
		...
		returns (uint newTimestamp)
	{
		...
		newTimestamp = _daysFromDate(
			year,
			month,
			day
		) * SECONDS_PER_DAY + timestamp % SECONDS_PER_DAY;
